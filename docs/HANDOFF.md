...
- **`npm run check` must be run bare — never piped.** A pipe reports the LAST command's exit code, so `npm run check | tail` says exit 0 while typecheck is failing and the tests never ran. This has now produced a false "543 passed" twice. If you want to keep the output, redirect: `npm run check > /tmp/check.log 2>&1; echo "EXIT=$?"`.
