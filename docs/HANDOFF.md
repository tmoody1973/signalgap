...
- **Do NOT fix `termsFor` without re-exporting the saved demo.** `convex/candidates/coverage.ts > termsFor` is wrong — it searches the initiating source's headline instead of the story (KNOWN-LIMITATIONS §2). The fix is known. But the saved demo is FROZEN data, so changing what coverage searches without producing a new fixture leaves the code and the demonstrated scan disagreeing about how coverage was decided. That is worse than the bug, because the bug is documented and the disagreement would be invisible. Fix it *and* re-scan, or leave it and keep the write-up.
...
- **`npm run check` must be run bare — never piped.** A pipe reports the LAST command's exit code, so `npm run check | tail -40` prints "exited with code 0" while typecheck is failing underneath. Happened on 2026-08-29. Redirect instead: `npm run check > /tmp/check.log 2>&1; echo "EXIT=$?"`.
